...
3. 创新点：we propose GraST, a geospatial-temporal semantic query optimization method that integrates property graphs and relational databases.
	- GraST是时空语义查询的优化方法
	- 它集成了图数据库和关系数据库，且利用了它们各自时空语义查询上的优势。
		- 图约束：用图数据库
		- 时空约束：用关系数据库
...
- 如何将一个查询，转化成对gdbms和rdbms的语句的组合？查询转换引擎：
	...
	- GDMS处理各个查询子命令
		- 若在GDBMS中能查询到，则直接查询
		- 若需要RDBMS，使用GDBMS Funtion Proxy，将使用GDBMS查询语句转化成sql语句，查询
	- 注意：下面这张图，是从yes开始的。
		- 查询中，有需要和不需要RDBMS的子查询
		- 把需要RDBMS的子查询，用GDBMS Function Proxy，转化成sql，向RDBMS查询
		- 把不需要RDBMS的子查询，直接在GDBMS中查询
		- 处理好后，用query executor执行所有语句
	- ![[Pasted image 20251228002141.png]]
顺一遍：
![[Pasted image 20251228005910.png]]

性能：
![[Pasted image 20251228010052.png]]
![[Pasted image 20251228010316.png]]

- conclusion：
	- 四种提速的方法
	- 比其他数据库都更快
	This paper proposes a geospatial-temporal semantic query optimization method called GraST. GraST adopts a lightweight hybrid storage architecture that bridges property graph databases and relational databases, combining the graph traversal and pattern matching capabilities of graph databases with the geographic computation capabilities of relational databases. It can simultaneously handle complex semantic constraints and spatiotemporal constraints, achieving flexible semantic modeling and efficient query analysis of geospatial-temporal data. Moreover, benefiting from the extensibility mechanisms of mainstream databases, GraST allows users to flexibly extend personalized spatiotemporal analysis operators by invoking the computation processes of underlying relational databases and spatial engines through custom function mappings. The loosely-coupled architecture design of GraST enables it to serve as a plug-and-play spatiotemporal semantic data management and query analysis engine that can be easily integrated into existing database platforms and IT assets. Developers only need to configure the connection information and query mappings in the adaptation layer to quickly empower existing systems with spatiotemporal semantic management capabilities, reducing system integration and secondary development costs. Experimental evaluations demonstrate that GraST exhibits excellent efficiency and scalability when handling complex spatiotemporal semantic queries on large-scale data, providing customizable and extensible high-performance geospatialtemporal computation capabilities for KGs. As research on spatially explicit KGs continues to deepen, the semantic integration of multi-source heterogeneous spatiotemporal data has become a key and challenging task. GraST achieves a balance between data uniformity and diversity, preserving the native formats and diversity of geographic data while utilizing KG techniques to realize unified management, integration, and semantic querying of heterogeneous data, providing a feasible solution for the integration of KGs and spatiotemporal data. In the future, GraST can be further combined with cutting-edge technologies such as knowledge ontologies, large language models, and deep learning to provide underlying data management and semantic query capabilities for GeoAI applications such as geographic knowledge intelligent question answering (Zhang et al., 2024; Huang and Harrie, 2019), spatiotemporal knowledge reasoning and decisionmaking.
