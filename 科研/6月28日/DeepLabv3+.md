...
空洞卷积的理解：
- 要提高“感受野”，目的是上下文整合，正常的操作是，池化或stride>1 + 正常卷积。这样的结果是输出分辨率比输入的更低，降低了精确定位。怎样才能保证输入输入分辨率相等（保持一定的精确定位），又能有大的“感受野”的？把卷积核做大，但导致计算量增常太多，这样不行。因此，得把卷积核做大，且其面用很多0占位，即空洞卷积。
- 空洞卷积用于DCNN后，能够在保证分辨率不变的情况下，通过调整rate来调整感受野的大小，实现类似于池化或者stride>1的卷积操作的效果。这种相对于池化和stride>1的效果，可以概括为得到密集特征图，保留了更多空间信息。但是负面效果就是计算的开销增大了。此外，保留原始特征分辨率进行的特点，结合其他的池化和stride，能够让我们得到任意分辨率的特征图。
...
（逐）深度可分离卷积（Depthwise separable convolution）：
- 效果上约等于标准卷积，但是有更低的计算复杂度
- 实现的原理是将标准卷积分成逐深度卷积和逐点卷积

空洞可分离卷积和深度可分离卷积的区别：
- 逐深度卷积，前者使用的atrous convolution的卷积核，后者使用的是标准卷积的卷积核心。
